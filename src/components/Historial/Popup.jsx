...
const Popup = ({ isPopupOpen, handlePopupClose, item }) => {
    ...
    const [selectedStatus, setSelectedStatus] = React.useState(item?.status || "");
                    ...
                    <div className="flex flex-col w-[50%] h-[50%] gap-1">
                        ...
                        <div className="w-full">
                            <textarea
                                value={item?.comments}
                                readOnly
                                className="w-full h-[8rem] bg-[#333] rounded-[10px] p-2 blanco h4 resize-none"
                            />
                        </div>
                        ...
                        <div className="w-full flex justify-end">
                            <button
                                className={"rojo-intenso-bg textos-peques2 blanco-suave rounded-[20px] py-2 px-4 flex items-center text-center " + (loading ? "opacity-50 cursor-not-allowed text-updating" : "")}
                                onClick={async () => {
                            ...
                            {item?.images?.length > 1 ? (
                                <Slider {...sliderSettings}>
                                    {item.images.map((imageObj) => (
                                        <div key={imageObj.id} className="flex justify-center items-center h-[200px]">
                                            <img
                                                src={imageObj.url}
